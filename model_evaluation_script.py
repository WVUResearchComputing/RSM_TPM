...
def CdSetup(MODEL_NAME,GSI_MODEL,ads_model,surf_part_mass,Inputs,mole_frac,datalength):
    ...
    if GSI_MODEL == 1: #DRIA
        ...
        Cd_srf = emu(srf_inputDRIA,MODEL_NAME,mole_frac,datalength,m_avg)
        ...
        ads_inputCLL[:,5] = srf_inputCLL[:,5] =abs( Inputs[:,3]) #Yaw (rad)
        ...
        #Emulator
        print('Beginning Emulator')
        Cd_ads = emu(ads_inputCLL,MODEL_NAME,mole_frac,datalength,m_avg)
        Cd_srf = emu(srf_inputCLL,MODEL_NAME,mole_frac,datalength,m_avg)
        #print(Cd_ads)
        #print(Cd_srf)
    else:
...
def emu(INPUT,MODEL_NAME,mole_frac,datalength,m_avg):
    ...
    #### Helium He
    # Loading up the trained GP model (Matern kernel) and the x,y normalization constants
    with open(MODEL_NAME +'_GP_reg_He_matern2.pkl', 'rb') as file_He:
        gp_He_Matern = pickle.load(file_He)
    ...
    #### Atomic Nitrogen N
    with open(MODEL_NAME + '_GP_reg_N_matern2.pkl', 'rb') as file_N:
        gp_N_Matern = pickle.load(file_N)
    ...
    # Loading up the trained GP model (Matern kernel) and the x,y normalization constants
    with open(MODEL_NAME + '_GP_reg_N2_matern2.pkl', 'rb') as file_N2:
        gp_N2_Matern = pickle.load(file_N2)
    ...
    #### Atomic Oxygen O
    # Loading up the trained GP model (Matern kernel) and the x,y normalization constants
    with open(MODEL_NAME + '_GP_reg_O_matern2.pkl', 'rb') as file_O:
        gp_O_Matern = pickle.load(file_O)
    ...
    #### Molecular Oxygen O2
    # Loading up the trained GP model (Matern kernel) and the x,y normalization constants
    with open(MODEL_NAME + '_GP_reg_O2_matern2.pkl', 'rb') as file_O2:
        gp_O2_Matern = pickle.load(file_O2)
...
if __name__ == '__main__':
   ...
   # basidr = os.join
    ...
    print('Preparing Projected Area Calculation...') 
    RSMA = area(basedir)
    #Save Area Results
    np.savetxt('Outputs/Projected_Area/'+MODEL_NAME+'_projectedarea.txt', RSMA)  
