...
######################## RUN TPMC ENSEMBLE #############################
def run_tpmc(args, NPROCS, speciesnames, ispec, rtype, outdir, RSMNAME):

    #RUN THE CODE
    print("Starting Simulation for "+speciesnames[ispec]+" "+rtype+" set\n")
    cmd = "%s -n %d %s" % (args.mpiexec, NPROCS, args.tpm)
...
def tpmc_loop(args, NPROCS, NENS,GSI_MODEL,Rotation, xmin,xmax, regdir, tpmdir,basedir, RSMNAME):
    ...
    for idx in range(2):  #Loop over to create test then training data
        for ispec in range(NSPECIES):   # Loop over each species of mole fraction for tpmc
            ...
            #Run TPMC code
            run_tpmc(args, NPROCS, speciesnames, ispec, rtype, outdir, RSMNAME)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mpiexec', metavar='NAME', type=str,
                            help='Path or name for MPI executor (mpirun or mpiexec) ')
    ...
    tpmc_loop(args,NPROCS, NENS,GSI_MODEL,Rotation,xmin,xmax, regdir, tpmdir,basedir, RSMNAME)
